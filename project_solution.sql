...
with total_spent_cte as (select sum(cast(amount as bigint)) as total_spent from credit_card)
...
from credit_card, total_spent_cte
...
select top 10 city,sum(amount) as total_spend
...
--5- write a query to print 3 columns:  Top 10 city, highest_expense_type , lowest_expense_type (example format : Delhi , bills, Fuel)
with cte as (
